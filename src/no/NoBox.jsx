  ...
  useEffect(() => {
    ...
    const move = () => {
      ...
      if (keysPressed.current['s'] && !keysPressed.current['w']) {
        newTop = Math.min(100 - (arrowRect.height / containerRect.height) * 100, position.top + 1);
      }
      ...
      if (keysPressed.current['d'] && !keysPressed.current['a']) {
        newLeft = Math.min(100 - (arrowRect.width / containerRect.width) * 100, position.left + 1);
      }
      ...
      const newCircles = circles.map(circle => {
        ...
        const circleRect = {
          ...
          bottom: (circleTop / 100) * containerRect.height + containerRect.top + 18,
        };
        ...
        if (isColliding) {
          if (circle.type === 'special') {
            ...
            return {
              ...
              top: Math.random() * 90,
              left: Math.random() * 90,
              color: circle.type === 'special' ? 'green' : 'blue',
              type: circle.type,
            };
          }
        }
      ...
      let finalCircles = removeChasers ? newCircles.filter(circle => circle.type !== 'chaser') : newCircles;
      ...
      if (!specialCircleExists && finalCircles.some(circle => circle.type === 'chaser')) {
        finalCircles.push({
          id: finalCircles.length,
          top: Math.random() * 90,
          left: Math.random() * 90,
